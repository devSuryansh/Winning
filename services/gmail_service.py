...
class GmailService:
    ...
    async def send_email(self, request: SendEmailRequest) -> SendEmailResponse:
        try:
            ...
            needs_auth = "oauth" in result_text.lower() or "authentication" in result_text.lower() or "clarification" in result_text.lower()
    ...
    async def send_automated_email(self, request: AutomatedEmailRequest) -> SendEmailResponse:
        try:
            ...
            result_text = str(result.get("result", ""))
            needs_auth = (result.get("needs_oauth", False) or 
                         "oauth" in result_text.lower() or 
                         "authentication" in result_text.lower() or 
                         "clarification" in result_text.lower())
